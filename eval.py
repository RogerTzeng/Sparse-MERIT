...
checkpoint = torch.load('./pretrained/WavLM-Large.pt', weights_only=True)
cfg = WavLMConfig(checkpoint['cfg'])
cfg.encoder_layerdrop = 0
ssl_model = WavLM(cfg)
ssl_model.load_state_dict(torch.load(MODEL_PATH+"/final_ssl.pt", weights_only=True)['model'])
...
if args.pooling_type in attention_pool_type_list:
    ...
    pool_model.load_state_dict(torch.load(MODEL_PATH+"/final_pool.pt", weights_only=True)['model'])
else:
...
##############################################
ser_model.load_state_dict(torch.load(MODEL_PATH+"/final_ser.pt", weights_only=True)['model'])
...
mmoe.load_state_dict(torch.load(MODEL_PATH+"/final_mmoe.pt", weights_only=True)['model'])
mmoe.eval(); mmoe.cuda()
