...
def MainModel_no_ws(args):
    
    model = BLSTM_multi_no_ws()
...
class BLSTM_multi_no_ws(nn.Module):
...
#         generate log1p
        (log1p,_phase),_len = self.transform(wav,ftype='log1p')
#         generate SSL feature
        B,T,embed_dim = layer_reps.shape
        layer_reps = layer_reps.repeat(1,1,2).reshape(B,-1,embed_dim)
        layer_reps = torch.cat((layer_reps,layer_reps[:,-1:].repeat(1,log1p.shape[2]-layer_reps.shape[1],1)),dim=1)
