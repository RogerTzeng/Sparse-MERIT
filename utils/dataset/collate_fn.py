...
def collate_fn_wav_lab_mask_data_dist(batch):
    total_wav = []
    total_lab = []
    total_dur = []
    total_utt = []
    ...
    for wav_data in batch:
        ...
        total_dur.append(dur)
        total_utt.append(wav_data[2])
        clean_wav, clean_dur = wav_data[3] 
        total_clean_wav.append(torch.Tensor(clean_wav))

    total_wav = nn.utils.rnn.pad_sequence(total_wav, batch_first=True)
    total_clean_wav = nn.utils.rnn.pad_sequence(total_clean_wav, batch_first=True)
    ...
    for data_idx, dur in enumerate(total_dur):
        attention_mask[data_idx,:dur] = 1
    ## compute mask
    return total_wav, total_lab, attention_mask, total_utt, total_clean_wav
